...
#Read in and prepare data for analysis.   
```{r}
co2_past <- read_table("https://cdiac.ess-dive.lbl.gov/ftp/trends/co2/vostok.icecore.co2", skip = 20, col_names = c("depth_m", "ice_age_yrBP", "air_mean_age_yrBP", "co2_conc_ppmv"))
...
co2_early <- co2 |> select(year, deseasonalized)
co2_early <- co2_early |> mutate(year_from_1950 = year - 1950)
co2_early <- co2_early |> rename(co2_conc = "deseasonalized")
co2_early <- co2_early |> select(year_from_1950, co2_conc)

#calculating year from 1950 for years before 1950 (ice core data)
co2_later <- co2_past |> select(air_mean_age_yrBP, co2_conc_ppmv)
co2_later <- co2_later |> mutate(year_from_1950 = 1950 - air_mean_age_yrBP)
co2_later <- co2_later |> rename(co2_conc = "co2_conc_ppmv")
